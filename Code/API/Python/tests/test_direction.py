...
class TestsDirection(unittest.TestCase):
    ...
    def test_repr_normal(self):
        ...
        self.assertTrue(repr(direction) == "Direction(x:4; y:-8; z:1)",
                        msg=f"__repr__() donne: {repr(direction)}")
    ...
    def test_repr_double_variable(self):
        ...
        with self.assertRaises(ValueError, msg="Avoir deux fois le même caractère n'a pas levé d'exception de type ValueError") as re:
            Direction("4x+4x+4y")
        self.assertEqual(str(re.exception),
                         "L'axe <x> apparaît plusieurs fois")

    def test_repr_null(self):
        """Test de la représentation d'une direction null"""
        direction = Direction("0")
        self.assertTrue(repr(direction) == "Direction(x:0; y:0; z:0)",
                        msg=f"La représentation d'une direction null donne: {repr(direction)}")
    ...
    def test_repr_deux_composantes(self):
        ...
        self.assertTrue(repr(direction) == "Direction(x:2; y:0; z:8)",
                        msg=f"La représentation d'une direction avec deux composantes donne: {repr(direction)}")

    def test_repr_distance_vecteur(self):
        """Test de la représentation d'une direction en vecteur"""
        direction = Direction("2x+8z")
        dir_vect = direction.vecteur()
        self.assertTrue(direction.axe_x == dir_vect.x,
                        msg=f"La composante en x de la direction en vecteur donne: {dir_vect.x}")
        self.assertTrue(direction.axe_y == dir_vect.y,
                        msg=f"La composante en y de la direction en vecteur donne: {dir_vect.y}")
        self.assertTrue(direction.axe_z == dir_vect.z,
                        msg=f"La composante en z de la direction en vecteur donne: {dir_vect.z}")

    def test_repr_reel(self):
        """Test de la représentation d'une direction avec des nombres réels"""
        direction = Direction("2.6x+8.1z")
        self.assertTrue(repr(direction) == "Direction(x:2.6; y:0; z:8.1)",
                        msg=f"La représentation d'une direction avec des réels donne: {repr(direction)}")
    ...
    def test_repr_vigule_reel(self):
        ...
        self.assertTrue(repr(direction) == "Direction(x:2.6; y:2; z:8.1)",
                        msg=f"La représentation d'une direction avec des virgules donne: {repr(direction)}")
