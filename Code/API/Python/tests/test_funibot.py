...
class TestsFunibot(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.config = Object()
        setattr(self.config, "liste_poteaux", self.poteaux)
        setattr(self.config, "sol", 0)
    
    def test_init(self):
        """Test d'initialisation d'un Funibot"""
        bot = Funibot(self.serial, config=self.config) # type: ignore
    ...
    def test_pos(self):
        bot = Funibot(self.dserial, config=self.config) # type: ignore
